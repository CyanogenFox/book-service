...
public class BookServiceImpl implements BookService {
	...
	public Iterable<String> findPublishersByAuthor(String authorName) {
		if (!authorRepository.existsById(authorName))
			throw new EntityNotFoundException();
		List<Book> books = bookRepository.findByAuthorsName(authorName).collect(Collectors.toList());
		return books.stream().map(b -> b.getPublisher().toString()).collect(Collectors.toSet());
	}
	...
	public AuthorDto removeAuthor(String authorName) {
		if (!authorRepository.existsById(authorName))
			throw new EntityNotFoundException();
		Set<Long> ids = bookRepository.findByAuthorsName(authorName).map(b -> b.getIsbn()).collect(Collectors.toSet());
